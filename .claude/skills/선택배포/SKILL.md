...
name: 선택배포
description: >
  Copies specified HTML files to deploy/ and publishes automatically.
...
- 사용자가 파일명만 말한 경우 (경로 없이): **반드시** 프로젝트 전체에서 같은 파일명을 검색하라.
  - 1개만 있으면 → 바로 진행.
  - **2개 이상 있으면 → 반드시 전체 목록을 경로와 함께 보여주고 선택을 받아라. 자동으로 선택하지 마라.**
...
- `archive/`에 같은 파일명이 있으면:
  - → "보관함에 같은 이름의 파일이 있어요: {파일명 목록}. 파일명을 변경하거나, 보관함의 파일을 먼저 삭제해주세요." 안내 후 중단.
- 둘 다 없으면 → 종류: **신규**
...
git add deploy/{파일1} deploy/{파일2} ...
# archive/ 소스가 있는 경우 (이동이므로 archive/ 삭제도 staging)
git add archive/{파일} ...
git commit -m "<커밋 메시지>"
...
**커밋 메시지:** `.claude/conventions.md`의 `deploy:` prefix 형식을 따르라.
